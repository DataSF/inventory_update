...
def main():
  ...
  for item in email_list[0:2]:
    msgBody  = base_email_txt % (item['coordinator_name'], item['template_file_name'])
    attachment_dictList = [{item['template_file_name']: wkbks_dir+item['template_file_name']}]
    ...
    mail_recipient = e.sendEmails(subject_line, msgBody, fname_attachment=None, fname_attachment_fullpath=None, recipients={ 'To': 'janine.heiser@sfgov.org', 'cc':'janine.heiser@sfgov.org', 'bcc': 'janine.heiser@sfgov.org'}, attachment_dictList = attachment_dictList, isETL=False)
